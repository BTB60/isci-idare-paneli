...
function getJwtSecret() {
  ...
  if (isProd) {
    if (!trimmed || trimmed.length < 32) {
      throw new Error(
        'JWT_SECRET production-da təyin olunmalıdır (≥32 simvol). ' +
          'Yerli: backend/.env və ya repo kökü .env — npm run gen:jwt-secrets (backend qovluğundan). ' +
          'Fly.io: fly secrets set JWT_SECRET=... JWT_REFRESH_SECRET=... MONGODB_URI=... ' +
          'Render.com (Docker/Web): Dashboard → Web Service → Environment — JWT_SECRET, JWT_REFRESH_SECRET, MONGODB_URI əlavə edin (.env image-da olmur).'
      );
    }
